...
class des:
	...
	def __des_crypt(self, block, crypt_type):
		...
		while i < 16:
			...
			i = i + 1
			iteration = iteration + iteration_adjustment
	...
	def crypt(self, data, crypt_type):
		...
		if len(data) % self.block_size != 0:
			...
			if not self.getPadding():
				...
				data = data + (self.block_size - (len(data) % self.block_size)) * self.getPadding()
	...
	try:
		...
		def unhex(s):
			...
			for i in range(0, len(s), 2):
				result.append(chr(string.atoi(s[i:i+2], 16)))
			return string.join(result, "")
